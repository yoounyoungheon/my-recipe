...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && email) {
      ...
      // 버전 정보를 정렬하여 상태에 저장
      const sortedVersionInfos = versionInfos.sort((a, b) => a.version - b.version);
      setVersionInfoList(sortedVersionInfos);

      // 최신 버전으로 레시피 설정
      const latestRecipe = myrecipes.reduce((prev, current) => {
        return (prev.version > current.version) ? prev : current;
      }, myrecipes[0]);
      
      setRecipe(latestRecipe);
      setVersion(latestRecipe.version);
    }
  }, [email, key, status]);
  ...
  return (
    <main>
      ...
      <div className="space-y-10">
        ...
        <Card className="w-full max-h-25">
          <CardContent className="space-y-4 pt-6 text-center">
            <div className="text-lg"><h1>Ingredients</h1></div>
            <div className="flex flex-wrap justify-center">
        ...
        <div className="flex justify-center space-x-4">
          ...
          <AchromaticButton onClick={() => { router.push(`/home`) }}>목록으로</AchromaticButton>
        </div>
      </div>
    </main>
  )
